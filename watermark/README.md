...
Detector đọc lại các block DCT đã được chọn bằng seed của user, vote theo
`redundancy`, sau đó vote ngang qua nhiều frame. Mặc định một candidate được
coi là phát hiện đúng khi bit recall >= 75%.

Kiểm tra precision với nhiều candidate:
...
python watermark/detect.py `
    ...
    --candidate-user-id user-fake-02
```

## Robustness benchmark

Để benchmark robustness, nên dùng cấu hình nhúng mạnh hơn demo mặc định:

```powershell
python watermark/embed.py `
    --in ingest/input/sample.mp4 `
    --out watermark/test-assets/sample_wm_robust.mp4 `
    --user-id user-24520228 `
    --frames 60 --frame-stride 5 `
    --strength 40 --redundancy 32
```

```powershell
python watermark/robustness.py `
    --in watermark/test-assets/sample_wm_robust.mp4 `
    --sidecar watermark/test-assets/sample_wm_robust.mp4.wm.json
```

Script sẽ tạo các biến thể:

| Attack | Mục tiêu kiểm thử |
|---|---|
| `reencode-crf23` | Watermark còn đọc được sau tái nén H.264 mức vừa. |
| `crop-4pct-resize` | Kiểm tra tác động khi lưới block 8x8 bị lệch hình học. |
| `blur-sigma1` | Kiểm tra mất hệ số tần số trung bình khi làm mờ. |
| `rotate-2deg` | Kiểm tra khi frame bị xoay nhẹ. |

Kết quả được ghi vào:

- `watermark/robustness_results.md` — bảng recall/precision để đưa vào báo cáo.
- `watermark/test-assets/robustness/robustness_results.csv` — raw CSV.
